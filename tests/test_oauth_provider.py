...
def test_crud_oauth_provider(client: TestClient):
    resp = client.get('/api/oauth_providers/')
    providers = resp.json()
    resp = client.post('/api/auth/login', json={'username': 'admin', 'password': 'admin'})
    assert resp.status_code == 200
    for i in providers:
        resp = client.delete(f'/api/oauth_providers/{i["id"]}')
        assert resp.status_code == 200
    resp = client.post('/api/oauth_providers/', json={
        'name': 'test_provider',
    ...
    resp = client.put(f'/api/oauth_providers/{provider_id}', json={
        'name': 'test_provider_updated',
    })
    assert resp.status_code == 200
    resp = client.get(f'/api/oauth_providers/{provider_id}')
    assert resp.status_code == 200
    assert resp.json()['name'] == 'test_provider_updated'
    resp = client.delete(f'/api/oauth_providers/{provider_id}')
    assert resp.status_code == 200