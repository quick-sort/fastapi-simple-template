...
def test_registry(client: TestClient):
    ...
    resp = client.post('/api/v1/auth/registry', json={'username': 'new_user', 'password': 'test', 'email': 'email@email.com'})
    assert resp.status_code == 200, 'registry failed'

    resp = client.post('/api/v1/auth/login', json={'username': 'new_user', 'password': 'test'})
...
def test_login(client: TestClient):
    ...
    resp = client.post('/api/v1/auth/login', json={'username': 'notexist', 'password': 'incorrect'})
    assert resp.status_code == 401, 'login check failed'

    resp = client.post('/api/v1/auth/login', json={'username': 'admin', 'password': 'incorrect'})
    assert resp.status_code == 401, 'login check failed'
...
def test_auth(client: TestClient):
    ...
    resp = client.get('/api/v1/auth/me', auth=httpx.BasicAuth('admin', 'admin'))
    assert resp.status_code == 200, 'basic auth middleware is not working'

    resp = client.get('/api/v1/auth/me', auth=httpx.BasicAuth('admin', 'incorrect'))
    ...
    resp = client.get('/api/v1/auth/me', headers={'Authorization': f'Bearer {access_token[0:10]}'})
...
def test_api_key(client: TestClient):
    ...
    api_key = 'incorrect'
    resp = client.get('/api/v1/auth/me', headers={'x-key': api_key})
    assert resp.status_code == 401, 'api key middleware is not working'

    resp = client.post('/api/v1/auth/login', json={'username': 'user', 'password': 'user'})
    assert resp.status_code == 200, 'login failed'

    resp = client.post('/api/v1/api_keys/my', params={'name': 'api_key'})
    ...
    resp = client.get('/api/v1/api_keys/my', headers={'x-key': api_key})
    assert resp.status_code == 200, 'api key middleware is not working'
    assert len(resp.json()) > 0

    resp = client.delete(f'/api/v1/api_keys/my/{key_id}', headers={'x-key': api_key})
    assert resp.status_code == 200, 'api key middleware is not working'
...
def test_change_password(client: TestClient):
    ...
    resp = client.post('/api/v1/auth/change_password', json={'old_password': 'incorrect', 'new_password': 'user'})
    assert resp.status_code == 401, 'failed to verify old password'

    resp = client.post('/api/v1/auth/change_password', json={'old_password': 'user', 'new_password': 'new_password'})
    assert resp.status_code == 200, 'failed to change password'

    resp = client.post('/api/v1/auth/login', json={'username': 'user', 'password': 'new_password'})
    assert resp.status_code == 200, 'failed to change password'

    resp = client.post('/api/v1/auth/change_password', json={'old_password': 'new_password', 'new_password': 'user'})
    assert resp.status_code == 200, 'failed to change password'