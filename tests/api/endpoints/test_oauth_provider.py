...
async def test_crud_oauth_provider(client: AsyncClient):
    ...
    providers = resp.json()
    for i in providers:
        resp = await client.delete(f"/api/v1/oauth/providers/{i['id']}")
        assert resp.status_code == 200
    resp = await client.post(
    ...
    resp = await client.patch(
        f"/api/v1/oauth/providers/{provider_id}",
        json={
            "name": "test_provider_updated",
        },
    )
    assert resp.status_code == 200
    resp = await client.get(f"/api/v1/oauth/providers/{provider_id}")
    assert resp.status_code == 200
    assert resp.json()["name"] == "test_provider_updated"
    resp = await client.delete(f"/api/v1/oauth/providers/{provider_id}")
    assert resp.status_code == 200
