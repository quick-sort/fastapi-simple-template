...
async def registry_user(
    ...
    users:list[User] = await User.find(async_session=db_session, username=params.username)
    if len(users) > 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="username exists",
        )
    users:list[User] = await User.find(async_session=db_session, email=params.email)
    if len(users) > 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="email exists",
        )
    user: User = await User.create(async_session=db_session, username=params.username, email=params.email, password=params.password, roles=[UserRole.user])
    return user
...
async def login(
    ...
    user:User | None = await User.find_one(async_session=db_session, username=params.username)
    if not user:
        raise HTTPException(
